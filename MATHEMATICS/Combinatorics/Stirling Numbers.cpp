...
ll stirlingNumber(ll n,ll k){
    ...
    if (k > n) dp[n][k] = -1;
    else if (k == 0) dp[n][k] = 0;
    else if (k == n) dp[n][k] = 1;
    else if (k == 1) dp[n][k] = dpFact[n-1];
    else if (n - k == 1) dp[n][k] = (n * (n-1) / 2 ) % MOD; 
    else dp[n][k] =  ( (stirlingNumber(n - 1, k - 1) % MOD) + ( ( ((n - 1)%MOD) * (stirlingNumber(n - 1, k)%MOD) )%MOD ) )%MOD; 
    return dp[n][k];
}
